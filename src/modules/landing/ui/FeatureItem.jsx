...
export default function FeatureItem({ icon, title, description }) {
  const handleClick = () => {
    eventBus.publish(events.FEATURE_CLICKED, { featureTitle: title });
  };

  return (
    <div 
      className="rounded-xl border-2 border-purple-custom bg-deep-space text-white shadow-feature-card hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] py-8 px-6 cursor-pointer group"
      onClick={handleClick}
    >
